 ...
 *
 * Does a signalled fence actually mean the GPU finished?
 */
#include "nvk_harness.h"

#define PATTERN    0xABCD1234u
#define STALE      0x00000000u
#define BUF_BYTES  (32u * 1024u * 1024u)   /* 32 MiB per buffer */
#define COPIES     16u                     /* chain length to keep the engine busy */
#define TAIL_WORDS 1024u                   /* last words written by the final copy */

int main(void)
{
   nvk_log_open("sdmc:/nvk_b2_flush.log");
   LOG("=== nvk_b2_flush ===");
   LOG("params: buf=%u MiB copies=%u", BUF_BYTES / (1024u * 1024u), COPIES);
   ...
   if (!CreateBuffer || !AllocateMemory || !MapMemory || !CmdFillBuffer ||
       !CmdCopyBuffer || !CmdPipelineBarrier || !CreateFence || !QueueSubmit ||
   ...
   CmdFillBuffer(cb, src, 0, BUF_BYTES, PATTERN);
   ...
   VkBufferCopy region = { .srcOffset = 0, .dstOffset = 0, .size = BUF_BYTES };
   for (uint32_t i = 0; i < COPIES; i++) {
      CmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT,
                         VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 1, &mb, 0, NULL, 0, NULL);
      if (i & 1) CmdCopyBuffer(cb, dst, src, 1, &region);
      else       CmdCopyBuffer(cb, src, dst, 1, &region);
   }
   if (COPIES & 1) {
      CmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT,
                         VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 1, &mb, 0, NULL, 0, NULL);
   ...
   const uint32_t nwords = BUF_BYTES / 4;
   const uint32_t *w = (const uint32_t *)dcpu;

   uint32_t tail_bad = 0, tail_first = 0;
   for (uint32_t i = nwords - TAIL_WORDS; i < nwords; i++)
      if (w[i] != PATTERN) { if (!tail_bad) tail_first = i; tail_bad++; }

   uint32_t bad = 0, first = 0;
   for (uint32_t i = 0; i < nwords; i++)
      if (w[i] != PATTERN) { if (!bad) first = i; bad++; }

   LOG("tail: %u/%u wrong (first tail word[%u]=0x%08x)",
       tail_bad, TAIL_WORDS, tail_first, w[tail_first]);

   if (bad == 0) {
      LOG("VERIFY OK: all %u words == 0x%08x after fence", nwords, PATTERN);
      LOG("=== nvk_b2_flush PASSED ===");
   } else {
      LOG("VERIFY FAIL: %u/%u words wrong. First word[%u]=0x%08x (expected 0x%08x)",
          bad, nwords, first, w[first], PATTERN);
      if (tail_bad)
         LOG("  fence signalled before the engine finished");
      else
         LOG("  body wrong but tail intact");
      LOG("=== nvk_b2_flush FAILED ===");
   }
